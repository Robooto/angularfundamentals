...
export class EventService {
  ...
  constructor(private http: Http) {}
    ...
    getEvents(): Observable<IEvent[]> {
        return this.http.get("http://localhost:8808/api/events")
          .map((response: Response) =>  <IEvent[]> response.json())
          .catch(this.handleError);
    }

    getEvent(id: number): Observable<IEvent>{
        return this.http.get(`http://localhost:8808/api/events/${id}`)
          .map((response: Response) =>  <IEvent> response.json())
          .catch(this.handleError);
    }
    ...
    saveEvent(event) {
      ...
      return this.http.post(`http://localhost:8808/api/events`, event, options)
        .map((response: Response) =>  <IEvent> response.json())
        .catch(this.handleError);
    }
    ...
    searchSessions(searchTerm){
      ...
      // }, 100);
      // return emitter;
      return this.http.get(`http://localhost:8808/api/sessions/search?search=${searchTerm}`)
